...
def parse_diff(diff_text: str) -> tuple[List[DiffBlock], List[DiffBlock], List[RenamedFile]]:
    ...
    A block is a contiguous run of added-only or removed-only lines,
    flushed when a context line (unchanged) is encountered. Blank lines
    inside a run are part of the block like any other line.

    This matches git's own notion of a change unit and works correctly
    for source code, markdown, and any other text format.
    """
        ...
        for hunk in patched_file:
            ...
            def flush_removed():
                ...
                if current_removed:
                    ...
                    removed_blocks.append(DiffBlock(
                        file_path, removed_start, content, False,
                        current_removed_raw.copy()
                    ))
            ...
            def flush_added():
                ...
                if current_added:
                    content = "\n".join(current_added)
                    added_blocks.append(DiffBlock(
                        file_path, added_start, content, True,
                        current_added_raw.copy()
                    ))
                current_added.clear()
